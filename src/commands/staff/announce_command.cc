...
bool shiro::commands::announce(std::deque<std::string>& args, const std::shared_ptr<shiro::users::user>& user, const std::string& channel) {
    ...
    if (!shiro::users::manager::has_permissions(user, permissions::perms::announce_cmd)) {
        utils::bot::respond("Permission denied. (Announce permission required)", user, channel, true);
        return false;
    }
