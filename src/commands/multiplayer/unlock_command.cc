...
#include "unlock_command.hh"

bool shiro::commands_mp::unlock(std::deque<std::string>& args, const std::shared_ptr<shiro::users::user>& user, const std::string& channel) {
    ...
    shiro::multiplayer::match_manager::iterate([&user, &channel](shiro::io::layouts::multiplayer_match& match) -> bool {
        ...
        if (match.host_id == user->user_id) {
            match.slots_locked = false;
            utils::bot::respond("Slots was unlocked.", user, channel, true);
            return true;
        }
